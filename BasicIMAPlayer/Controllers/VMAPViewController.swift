...
class VMAPViewController: BaseViewController {
    ...
    override func setupPlaybackController() {
        ...
        imaSettings.ppid = PlayerConfig.IMAConfig.PublisherID
        ...
        if (useVideoProperties) {
            ...
            adsRequestPolicy = BCOVIMAAdsRequestPolicy.init(vmapAdTagUrl: PlayerConfig.IMAConfig.VMAPAdTagURL)
        }
    ...
    override func updateVideo(_ video: BCOVVideo) -> BCOVVideo {
        if (useVideoProperties) {
            return video.updateVideo(withVMAPTag: PlayerConfig.IMAConfig.VMAPAdTagURL)
        }
        return video
    }
